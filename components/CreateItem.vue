    ...
    <div class="modal px-8 pb-12">
      <v-row class="py-4 px-4">
        <v-col sm="12" lg="6" md="6" cols="12">
          <v-row>
            <v-btn text class="mr-4" @click="toggleCreate(false)">
              <i class="material-icons">arrow_back</i>
            </v-btn>
            <h1 class="display-1 font-bold">Create New List</h1>
          </v-row>
        </v-col>
        <v-col sm="12" lg="6" md="6" cols="12">
          <v-row justify="end">
            <v-btn class="mr-4" outlined color="pink">Save as Draft</v-btn>
            <v-btn color="pink" class="white--text" v-if="loading">...</v-btn>
            <v-btn v-else color="pink" class="white--text" @click="addList">Publish</v-btn>
          </v-row>
        </v-col>
      </v-row>
      <div class="px-4">
  ...
  methods: {
    ...
    addList() {
      if (
        ...
        this.$store.dispatch("categories/addList", {
          list: {
            title: this.title,
